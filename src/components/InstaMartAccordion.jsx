...
function InstaMartAccordion({ title }) {
    const [isDescVisible, setIsDesVisible] = useState(false);
    return (
        <div className="border p-5">
            <p className="font-bold text-2xl">{title}</p>
            <button onClick={() => setIsDesVisible(!isDescVisible)} className="underline font-semibold">{isDescVisible ? "Hide" : "Show"}</button>
            {isDescVisible &&
